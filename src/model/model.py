...
def initConsts():
    ...
    constants[50] = constants[48]*np.log(constants[8]/constants[7])
    constants[51] = constants[48]*np.log(constants[28]/constants[13])
    constants[52] = constants[48]*np.log(constants[8]/constants[7])
...
def computeRates(voi, states, constants):
    ...
    algebraic[15] = 120.000/(1.00000+np.exp(-(V+50.0000)/15.0000))
    rates[17] = constants[42]*(1.00000-achf)-algebraic[15]*achf
    algebraic[16] = 5.82000/(1.00000+np.exp(-(V+50.0000)/15.0000))
    rates[18] = constants[43]*(1.00000-achs)-algebraic[16]*achs
    algebraic[18] = 2277.00*2.50000*((1.00000-f_TMC)-f_TMM)-751.000*f_TMM
    rates[24] = algebraic[18]
    algebraic[0] = 1.00000/(1.00000+np.exp(((V+83.1900)-(-7.20000*(np.power(constants[5], 0.690000)))/(np.power(1.26000e-05, 0.690000)+np.power(constants[5], 0.690000)))/13.5600))
    algebraic[19] = 0.250000+2.00000*np.exp(-(np.power(V+70.0000, 2.00000))/500.000)
    rates[1] = (algebraic[0]-y)/algebraic[19]
    algebraic[1] = 1.00000/(1.00000+np.exp((V+10.2200)/-8.50000))
    algebraic[20] = 1.00000/(17.0000*np.exp(0.0398000*V)+0.211000*np.exp(-0.0510000*V))
    rates[2] = (algebraic[1]-paf)/algebraic[20]
    algebraic[2] = 1.00000/(1.00000+np.exp((V+10.2200)/-8.50000))
    algebraic[21] = 0.335810+0.906730*np.exp(-(np.power(V+10.0000, 2.00000))/988.050)
    rates[3] = (algebraic[2]-pas)/algebraic[21]
    algebraic[8] = 1.00000/(1.00000+np.exp((V-(-24.0000+constants[35]))/6.31000))
    algebraic[27] = 0.0100000+0.153900*np.exp(-(np.power(V+40.0000, 2.00000))/185.670)
    rates[10] = (algebraic[8]-f)/algebraic[27]
    algebraic[9] = 1.00000/(1.00000+np.exp((V-(-24.0000+constants[36]))/6.31000))
    algebraic[28] = 0.0600000+0.480760*2.25000*np.exp(-(np.power(V--40.0000, 2.00000))/138.040)
    rates[11] = (algebraic[9]-f2)/algebraic[28]
    algebraic[29] = 0.000596000+0.00311800/(1.03700*np.exp(0.0900000*(V+30.6100))+0.396000*np.exp(-0.120000*(V+23.8400)))
    algebraic[10] = 1.00000/(1.00000+np.exp((V-7.44000)/-16.4000))
    rates[12] = (algebraic[10]-r)/algebraic[29]
    algebraic[30] = 0.0126600+4.72716/(1.00000+np.exp((V+154.500)/23.9600))
    algebraic[11] = 1.00000/(1.00000+np.exp((V+33.8000)/6.12000))
    rates[13] = (algebraic[11]-q_fast)/algebraic[30]
    algebraic[31] = 0.100000+4.00000*np.exp(-(np.power(V+65.0000, 2.00000))/500.000)
    algebraic[12] = 1.00000/(1.00000+np.exp((V+33.8000)/6.12000))
    rates[14] = (algebraic[12]-q_slow)/algebraic[31]
    algebraic[4] = V+44.4000
    algebraic[23] = custom_piecewise([np.less(np.fabs(algebraic[4]) , constants[30]), (-460.000*-12.6730)/np.exp(algebraic[4]/-12.6730) , True, (-460.000*algebraic[4])/(np.exp(algebraic[4]/-12.6730)-1.00000)])
    algebraic[36] = 18400.0*np.exp(algebraic[4]/-12.6730)
    rates[6] = algebraic[23]*(1.00000-m)-algebraic[36]*m
    algebraic[3] = (1.00000/(1.00000+np.exp((V+4.90000)/15.1400)))*(1.00000-0.300000*np.exp(-(np.power(V, 2.00000))/500.000))
    algebraic[22] = 92.0100*np.exp(-0.0183000*V)
    algebraic[35] = 603.600*np.exp(0.00942000*V)
    algebraic[43] = 1.00000/(algebraic[22]+algebraic[35])
    rates[4] = (algebraic[3]-pik)/algebraic[43]
    algebraic[5] = 44.9000*np.exp((V+66.9000)/-5.57000)
    algebraic[24] = 1491.00/(1.00000+323.300*np.exp((V+94.6000)/-12.9000))
    algebraic[37] = algebraic[5]/(algebraic[5]+algebraic[24])
    algebraic[44] = 0.0300000/(1.00000+np.exp((V+40.0000)/6.00000))+0.000350000
    rates[7] = (algebraic[37]-h1)/algebraic[44]
    algebraic[6] = 44.9000*np.exp((V+66.9000)/-5.57000)
    algebraic[25] = 1491.00/(1.00000+323.300*np.exp((V+94.6000)/-12.9000))
    algebraic[38] = algebraic[6]/(algebraic[6]+algebraic[25])
    algebraic[45] = 0.120000/(1.00000+np.exp((V+60.0000)/2.00000))+0.00295000
    rates[8] = (algebraic[38]-h2)/algebraic[45]
    algebraic[39] = 1.00000/(1.00000+np.exp((V-(-3.20000+constants[33]))/constants[34]))
    algebraic[7] = (-26.1200*(V+35.0000))/(np.exp((V+35.0000)/-2.50000)-1.00000)+(-78.1100*V)/(np.exp(-0.208000*V)-1.00000)
    algebraic[26] = (10.5200*(V-5.00000))/(np.exp(0.400000*(V-5.00000))-1.00000)
    algebraic[46] = 1.00000/(algebraic[7]+algebraic[26])
    rates[9] = (algebraic[39]-d)/algebraic[46]
    algebraic[32] = 1.00000/(0.150000*np.exp(-V/11.0000)+0.200000*np.exp(-V/700.000))
    algebraic[40] = 1.00000/(16.0000*np.exp(V/8.00000)+15.0000*np.exp(V/50.0000))
    algebraic[47] = 0.00100000/(algebraic[32]+algebraic[40])
    algebraic[13] = 1.00000/(1.00000+np.exp((V--49.1000)/-8.98000))
    rates[15] = (algebraic[13]-qa)/algebraic[47]
    algebraic[14] = 0.150400/(3100.00*np.exp(V/13.0000)+700.000*np.exp(V/70.0000))
    algebraic[33] = 0.150400/(95.0000*np.exp(-V/10.0000)+50.0000*np.exp(-V/700.000))+0.000229000/(1.00000+np.exp(-V/5.00000))
    algebraic[48] = 0.00100000/(algebraic[14]+algebraic[33])
    algebraic[41] = algebraic[14]/(algebraic[14]+algebraic[33])
    rates[16] = (algebraic[41]-qi)/algebraic[48]
    algebraic[65] = (((constants[29]*(np.power(m, 3.00000))*(0.635000*h1+0.365000*h2)*constants[28]*V*constants[2])/constants[48])*(np.exp((V-constants[51])/constants[48])-1.00000))/(np.exp(V/constants[48])-1.00000)
    algebraic[68] = (constants[40]*achf*achs*(np.power(constants[5], 1.50000)))/(np.power(constants[41], 1.50000)+np.power(constants[5], 1.50000))
    algebraic[69] = (((algebraic[68]*constants[8])/(10.0000+constants[8]))*(V-constants[50]))/(1.00000+np.exp(((V-constants[50])-140.000)/(2.50000*constants[48])))
    algebraic[70] = constants[31]*d*(0.675000*f+0.325000*f2)*(V-constants[32])*(1.00000-((algebraic[69]*constants[5])/(9.00000e-05+constants[5]))/1.00000)
    algebraic[66] = constants[37]*r*(0.450000*q_fast+0.550000*q_slow)*(V-constants[52])
    algebraic[34] = constants[6]*(0.900000*paf+0.100000*pas)*pik*(V-constants[50])
    algebraic[17] = y*constants[4]*(V--30.0000)
    algebraic[67] = constants[39]*qa*qi*(V-constants[38])
    algebraic[42] = constants[9]*(0.500000+0.500000/(1.00000+np.exp((V+30.0000)/5.00000)))
    algebraic[49] = (algebraic[42]*(np.power(constants[8]/(constants[8]+0.590000), 3.00000))*(V+81.9000))/(1.00000+np.exp((1.39300*(V+81.9000+3.60000))/constants[48]))
    algebraic[57] = np.exp((-constants[16]*V)/(2.00000*constants[48]))
    algebraic[52] = 1.00000+(constants[27]/constants[26])*(1.00000+np.exp((constants[17]*V)/constants[48]))+constants[28]/constants[24]+(np.power(constants[28], 2.00000))/(constants[24]*constants[25])+(np.power(constants[28], 3.00000))/(constants[24]*constants[25]*constants[23])
    algebraic[54] = (((np.power(constants[28], 2.00000))/(constants[24]*constants[25])+(np.power(constants[28], 3.00000))/(constants[24]*constants[25]*constants[23]))*np.exp((-constants[16]*V)/(2.00000*constants[48])))/algebraic[52]
    algebraic[55] = ((constants[27]/constants[26])*np.exp((-constants[17]*V)/constants[48]))/algebraic[52]
    algebraic[53] = np.exp((constants[16]*V)/(2.00000*constants[48]))
    algebraic[60] = algebraic[57]*constants[49]*(algebraic[54]+algebraic[55])+algebraic[55]*algebraic[53]*(constants[53]+algebraic[57])
    algebraic[56] = 1.00000+(Casub/constants[18])*(1.00000+np.exp((-constants[15]*V)/constants[48])+constants[13]/constants[22])+constants[13]/constants[19]+(np.power(constants[13], 2.00000))/(constants[19]*constants[20])+(np.power(constants[13], 3.00000))/(constants[19]*constants[20]*constants[21])
    algebraic[59] = ((Casub/constants[18])*np.exp((-constants[15]*V)/constants[48]))/algebraic[56]
    algebraic[58] = (((np.power(constants[13], 2.00000))/(constants[19]*constants[20])+(np.power(constants[13], 3.00000))/(constants[19]*constants[20]*constants[21]))*np.exp((constants[16]*V)/(2.00000*constants[48])))/algebraic[56]
    ...
    algebraic[51] = (constants[12]*(np.power(constants[13]/(5.64000+constants[13]), 3.00000))*(np.power(constants[8]/(0.621000+constants[8]), 2.00000))*1.60000)/(1.50000+np.exp(-(V+60.0000)/40.0000))
    algebraic[50] = constants[10]*(V-constants[11])
    ...
    algebraic[72] = 5.00000/(1.00000+constants[46]/Cai)
    algebraic[73] = (Ca_up-Ca_rel)/constants[47]
    rates[20] = algebraic[72]-(algebraic[73]*constants[55])/constants[54]
    algebraic[74] = 88800.0*Cai*(1.00000-f_TC)-446.000*f_TC
    rates[22] = algebraic[74]
    algebraic[76] = 227700.*Cai*((1.00000-f_TMC)-f_TMM)-7.51000*f_TMC
    rates[23] = algebraic[76]
    algebraic[75] = (constants[45]*(Ca_rel-Casub))/(1.00000+np.power(0.00120000/Casub, 2.00000))
    algebraic[79] = 534.000*Ca_rel*(1.00000-f_CQ)-445.000*f_CQ
    rates[21] = (algebraic[73]-algebraic[75])-10.0000*algebraic[79]
    algebraic[71] = (Casub-Cai)/4.00000e-05
    algebraic[77] = 227700.*Cai*(1.00000-f_CMi)-542.000*f_CMi
    ...
    algebraic[78] = 227700.*Casub*(1.00000-f_CMs)-542.000*f_CMs
    rates[26] = algebraic[78]
    rates[27] = algebraic[79]
    algebraic[80] = 0.00100000*(115.000*Casub*(1.00000-f_CSL)-1000.00*f_CSL)
    ...
    count_fibroblasts = constants[58]
    ...
    g_gap = constants[59]  # nano Simens
    ...
    I_kv_fib = g_Kv_fib * r_Kv_fib * s_Kv_fib * (V_cfi - E_k_fib)  # pA

    # E_K_fib = R * T / F * log(K_o_fib / K_i_fib)
    alpha_K1_fib = 0.1 / (1.0 + np.exp(0.06 * (V_cfi - E_k_fib - 200.0)))
    beta_K1_fib = (
                          3.0 * np.exp(0.0002 * (V_cfi - E_k_fib + 100.0)) + np.exp(0.1 * (V_cfi - E_k_fib - 10.0))
                  ) / (1.0 + np.exp(-0.5 * (V_cfi - E_k_fib)))

    I_K1_fib = (
            g_K1_fib * alpha_K1_fib * (V_cfi - E_k_fib) / (alpha_K1_fib + beta_K1_fib)
    )  # ok #pA

    I_NaK_fib = (
            I_NaK_fib_max
            * (K_o_fib / (K_o_fib + K_mK_fib))
            * (Na_i_fib ** 1.5 / (Na_i_fib ** 1.5 + K_mNa_fib ** 1.5))
            * (V_cfi - V_rev_fib)
            / (V_cfi - B_fib)
    )  # ok #pA

    E_Na_fib = R * T / F * np.log(Na_o_fib / Na_i_fib)
    I_bNa_fib = g_bNa_fib * (V_cfi - E_Na_fib)  # pust ok #pA
    rates[32] = -1.0 * (I_bNa_fib + 3.0 * I_NaK_fib) / (1.0 * V_c_fib * F) * Cm_fi

    i_gap = g_gap * (V_cfi - V)
    i_gap_ = g_gap * (V_cfi - V) / Cm_fi
    i_gap2 = g_gap * (V - V_cfi)
    ...
    rates[29] = (
            -1.0
            / 1.0
            * (I_kv_fib + I_K1_fib + I_NaK_fib + I_bNa_fib + i_mech_sens / Cm_fi + i_gap / Cm_fi)
    )
    tau_r_fib = 20.3 + 138.0 * np.exp(-(((V_cfi + 20.0) / 25.9) ** 2))
    tau_s_fib = 1574.0 + 5268.0 * np.exp(-(((V_cfi + 23.0) / 22.7) ** 2))
    r_fib = 1.0 / (1 + np.exp(-(V_cfi + 20.0) / 11.0))
    s_fib = 1.0 / (1 + np.exp(V_cfi + 23.0 / 7.0))
    rates[30] = (-r_Kv_fib + r_fib) / tau_r_fib
    rates[31] = (-s_Kv_fib + s_fib) / tau_s_fib
    dK_i_fib = -1.0 * (I_K1_fib + I_kv_fib - 2.0 * I_NaK_fib) / (1.0 * V_c_fib * F) * Cm_fi

    rates[0] = -(algebraic[65]+algebraic[70]+algebraic[66]+algebraic[34]+algebraic[17]+algebraic[67]+algebraic[49]+algebraic[64]+algebraic[51]+algebraic[50]+algebraic[69]+ count_fibroblasts * i_gap2/1000)/constants[3]
...
def computeAlgebraic(constants, states, voi):
    ...
    voi = np.array(voi)
    algebraic[15] = 120.000/(1.00000+np.exp(-(states[0]+50.0000)/15.0000))
    algebraic[16] = 5.82000/(1.00000+np.exp(-(states[0]+50.0000)/15.0000))
    algebraic[18] = 2277.00*2.50000*((1.00000-states[23])-states[24])-751.000*states[24]
    algebraic[0] = 1.00000/(1.00000+np.exp(((states[0]+83.1900)-(-7.20000*(np.power(constants[5], 0.690000)))/(np.power(1.26000e-05, 0.690000)+np.power(constants[5], 0.690000)))/13.5600))
    algebraic[19] = 0.250000+2.00000*np.exp(-(np.power(states[0]+70.0000, 2.00000))/500.000)
    algebraic[1] = 1.00000/(1.00000+np.exp((states[0]+10.2200)/-8.50000))
    algebraic[20] = 1.00000/(17.0000*np.exp(0.0398000*states[0])+0.211000*np.exp(-0.0510000*states[0]))
    algebraic[2] = 1.00000/(1.00000+np.exp((states[0]+10.2200)/-8.50000))
    algebraic[21] = 0.335810+0.906730*np.exp(-(np.power(states[0]+10.0000, 2.00000))/988.050)
    algebraic[8] = 1.00000/(1.00000+np.exp((states[0]-(-24.0000+constants[35]))/6.31000))
    algebraic[27] = 0.0100000+0.153900*np.exp(-(np.power(states[0]+40.0000, 2.00000))/185.670)
    algebraic[9] = 1.00000/(1.00000+np.exp((states[0]-(-24.0000+constants[36]))/6.31000))
    algebraic[28] = 0.0600000+0.480760*2.25000*np.exp(-(np.power(states[0]--40.0000, 2.00000))/138.040)
    algebraic[29] = 0.000596000+0.00311800/(1.03700*np.exp(0.0900000*(states[0]+30.6100))+0.396000*np.exp(-0.120000*(states[0]+23.8400)))
    algebraic[10] = 1.00000/(1.00000+np.exp((states[0]-7.44000)/-16.4000))
    algebraic[30] = 0.0126600+4.72716/(1.00000+np.exp((states[0]+154.500)/23.9600))
    algebraic[11] = 1.00000/(1.00000+np.exp((states[0]+33.8000)/6.12000))
    algebraic[31] = 0.100000+4.00000*np.exp(-(np.power(states[0]+65.0000, 2.00000))/500.000)
    algebraic[12] = 1.00000/(1.00000+np.exp((states[0]+33.8000)/6.12000))
    algebraic[4] = states[0]+44.4000
    algebraic[23] = custom_piecewise([np.less(np.fabs(algebraic[4]) , constants[30]), (-460.000*-12.6730)/np.exp(algebraic[4]/-12.6730) , True, (-460.000*algebraic[4])/(np.exp(algebraic[4]/-12.6730)-1.00000)])
    algebraic[36] = 18400.0*np.exp(algebraic[4]/-12.6730)
    algebraic[3] = (1.00000/(1.00000+np.exp((states[0]+4.90000)/15.1400)))*(1.00000-0.300000*np.exp(-(np.power(states[0], 2.00000))/500.000))
    algebraic[22] = 92.0100*np.exp(-0.0183000*states[0])
    algebraic[35] = 603.600*np.exp(0.00942000*states[0])
    algebraic[43] = 1.00000/(algebraic[22]+algebraic[35])
    algebraic[5] = 44.9000*np.exp((states[0]+66.9000)/-5.57000)
    algebraic[24] = 1491.00/(1.00000+323.300*np.exp((states[0]+94.6000)/-12.9000))
    algebraic[37] = algebraic[5]/(algebraic[5]+algebraic[24])
    algebraic[44] = 0.0300000/(1.00000+np.exp((states[0]+40.0000)/6.00000))+0.000350000
    algebraic[6] = 44.9000*np.exp((states[0]+66.9000)/-5.57000)
    algebraic[25] = 1491.00/(1.00000+323.300*np.exp((states[0]+94.6000)/-12.9000))
    algebraic[38] = algebraic[6]/(algebraic[6]+algebraic[25])
    algebraic[45] = 0.120000/(1.00000+np.exp((states[0]+60.0000)/2.00000))+0.00295000
    algebraic[39] = 1.00000/(1.00000+np.exp((states[0]-(-3.20000+constants[33]))/constants[34]))
    algebraic[7] = (-26.1200*(states[0]+35.0000))/(np.exp((states[0]+35.0000)/-2.50000)-1.00000)+(-78.1100*states[0])/(np.exp(-0.208000*states[0])-1.00000)
    algebraic[26] = (10.5200*(states[0]-5.00000))/(np.exp(0.400000*(states[0]-5.00000))-1.00000)
    algebraic[46] = 1.00000/(algebraic[7]+algebraic[26])
    algebraic[32] = 1.00000/(0.150000*np.exp(-states[0]/11.0000)+0.200000*np.exp(-states[0]/700.000))
    algebraic[40] = 1.00000/(16.0000*np.exp(states[0]/8.00000)+15.0000*np.exp(states[0]/50.0000))
    algebraic[47] = 0.00100000/(algebraic[32]+algebraic[40])
    algebraic[13] = 1.00000/(1.00000+np.exp((states[0]--49.1000)/-8.98000))
    algebraic[14] = 0.150400/(3100.00*np.exp(states[0]/13.0000)+700.000*np.exp(states[0]/70.0000))
    algebraic[33] = 0.150400/(95.0000*np.exp(-states[0]/10.0000)+50.0000*np.exp(-states[0]/700.000))+0.000229000/(1.00000+np.exp(-states[0]/5.00000))
    algebraic[48] = 0.00100000/(algebraic[14]+algebraic[33])
    algebraic[41] = algebraic[14]/(algebraic[14]+algebraic[33])
    algebraic[65] = (((constants[29]*(np.power(states[6], 3.00000))*(0.635000*states[7]+0.365000*states[8])*constants[28]*states[0]*constants[2])/constants[48])*(np.exp((states[0]-constants[51])/constants[48])-1.00000))/(np.exp(states[0]/constants[48])-1.00000)
    algebraic[68] = (constants[40]*states[17]*states[18]*(np.power(constants[5], 1.50000)))/(np.power(constants[41], 1.50000)+np.power(constants[5], 1.50000))
    algebraic[69] = (((algebraic[68]*constants[8])/(10.0000+constants[8]))*(states[0]-constants[50]))/(1.00000+np.exp(((states[0]-constants[50])-140.000)/(2.50000*constants[48])))
    ...
    algebraic[42] = constants[9]*(0.500000+0.500000/(1.00000+np.exp((states[0]+30.0000)/5.00000)))
    algebraic[49] = (algebraic[42]*(np.power(constants[8]/(constants[8]+0.590000), 3.00000))*(states[0]+81.9000))/(1.00000+np.exp((1.39300*(states[0]+81.9000+3.60000))/constants[48]))
    algebraic[57] = np.exp((-constants[16]*states[0])/(2.00000*constants[48]))
    algebraic[52] = 1.00000+(constants[27]/constants[26])*(1.00000+np.exp((constants[17]*states[0])/constants[48]))+constants[28]/constants[24]+(np.power(constants[28], 2.00000))/(constants[24]*constants[25])+(np.power(constants[28], 3.00000))/(constants[24]*constants[25]*constants[23])
    algebraic[54] = (((np.power(constants[28], 2.00000))/(constants[24]*constants[25])+(np.power(constants[28], 3.00000))/(constants[24]*constants[25]*constants[23]))*np.exp((-constants[16]*states[0])/(2.00000*constants[48])))/algebraic[52]
    algebraic[55] = ((constants[27]/constants[26])*np.exp((-constants[17]*states[0])/constants[48]))/algebraic[52]
    algebraic[53] = np.exp((constants[16]*states[0])/(2.00000*constants[48]))
    algebraic[60] = algebraic[57]*constants[49]*(algebraic[54]+algebraic[55])+algebraic[55]*algebraic[53]*(constants[53]+algebraic[57])
    algebraic[56] = 1.00000+(states[5]/constants[18])*(1.00000+np.exp((-constants[15]*states[0])/constants[48])+constants[13]/constants[22])+constants[13]/constants[19]+(np.power(constants[13], 2.00000))/(constants[19]*constants[20])+(np.power(constants[13], 3.00000))/(constants[19]*constants[20]*constants[21])
    algebraic[59] = ((states[5]/constants[18])*np.exp((-constants[15]*states[0])/constants[48]))/algebraic[56]
    algebraic[58] = (((np.power(constants[13], 2.00000))/(constants[19]*constants[20])+(np.power(constants[13], 3.00000))/(constants[19]*constants[20]*constants[21]))*np.exp((constants[16]*states[0])/(2.00000*constants[48])))/algebraic[56]
    algebraic[61] = algebraic[53]*constants[53]*(algebraic[58]+algebraic[59])+algebraic[57]*algebraic[59]*(constants[49]+algebraic[53])
    algebraic[62] = algebraic[58]*constants[53]*(algebraic[54]+algebraic[55])+algebraic[59]*algebraic[54]*(constants[53]+algebraic[57])
    algebraic[63] = algebraic[54]*constants[49]*(algebraic[58]+algebraic[59])+algebraic[58]*algebraic[55]*(constants[49]+algebraic[53])
    algebraic[64] = (constants[14]*(algebraic[61]*algebraic[55]-algebraic[60]*algebraic[59]))/(algebraic[60]+algebraic[61]+algebraic[62]+algebraic[63])
    algebraic[51] = (constants[12]*(np.power(constants[13]/(5.64000+constants[13]), 3.00000))*(np.power(constants[8]/(0.621000+constants[8]), 2.00000))*1.60000)/(1.50000+np.exp(-(states[0]+60.0000)/40.0000))
    ...
    algebraic[75] = (constants[45]*(states[21]-states[5]))/(1.00000+np.power(0.00120000/states[5], 2.00000))
